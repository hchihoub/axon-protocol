#!/usr/bin/env npx tsx
/**
 * AXON Extensions Manager Server — MCP-Compatible Stdio Wrapper
 *
 * Translates MCP JSON-RPC over stdin/stdout into AXON calls.
 * This makes the AXON Extensions Manager server usable with any MCP client:
 *   - Claude Desktop
 *   - Cursor
 *   - Windsurf
...
async function handleNotification(msg: MCPNotification): Promise<void> {
  if (msg.method === "notifications/initialized") {
    if (!em?.isLaunched) {
      ...
      const created = createExtensionsManagerServer(em);
      server = created.server;
      store = created.store;
      capAuthority = created.capAuthority;
    }
    initialized = true;
    log("Chrome launched with user profile, extensions manager server ready");
  }
...
function log(...args: any[]): void {
  process.stderr.write(`[axon-extensions] ${args.join(" ")}\n`);
}

async function main(): Promise<void> {
  log("Starting AXON Extensions Manager MCP server (stdio)...");
