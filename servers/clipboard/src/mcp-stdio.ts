#!/usr/bin/env npx tsx
/**
 * AXON Clipboard Server — MCP-Compatible Stdio Wrapper
 *
 * Translates MCP JSON-RPC over stdin/stdout into AXON calls.
 * This makes the AXON Clipboard server usable with any MCP client:
 *   - Claude Desktop
 *   - Cursor
 *   - Windsurf
 ...
 *   - AXON_CLIPBOARD_MAX_HISTORY: Maximum history entries (default: 100)
 *   - AXON_CLIPBOARD_HISTORY_PATH: Custom history file path
 */
...
function initServer(): void {
  cm = new ClipboardManager({
    maxHistory: parseInt(process.env.AXON_CLIPBOARD_MAX_HISTORY ?? "100", 10),
    historyPath: process.env.AXON_CLIPBOARD_HISTORY_PATH || undefined,
  });

  const created = createClipboardServer(cm);
...
function log(...args: any[]): void {
  process.stderr.write(`[axon-clipboard] ${args.join(" ")}\n`);
}

async function main(): Promise<void> {
  log("Starting AXON Clipboard MCP server (stdio)...");

  // Pre-initialize so tools are available immediately
  initServer();
