#!/usr/bin/env npx tsx
/**
 * AXON Tab Session Manager Server — MCP-Compatible Stdio Wrapper
 *
 * Translates MCP JSON-RPC over stdin/stdout into AXON calls.
 * This makes the AXON Tab Session Manager server usable with any MCP client:
 *   - Claude Desktop
 *   - Cursor
 *   - Windsurf
...
async function handleNotification(msg: MCPNotification): Promise<void> {
  if (msg.method === "notifications/initialized") {
    if (!tsm?.isLaunched) {
      ...
      const created = createTabSessionManagerServer(tsm);
      server = created.server;
      store = created.store;
      capAuthority = created.capAuthority;
    }
    initialized = true;
    log("Chrome launched with user profile, tab session manager server ready");
  }
...
function log(...args: any[]): void {
  process.stderr.write(`[axon-tabs] ${args.join(" ")}\n`);
}

async function main(): Promise<void> {
  log("Starting AXON Tab Session Manager MCP server (stdio)...");
