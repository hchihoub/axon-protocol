#!/usr/bin/env npx tsx
/**
 * AXON ENV/Secrets Vault Server — MCP-Compatible Stdio Wrapper
 *
 * Translates MCP JSON-RPC over stdin/stdout into AXON calls.
 * This makes the AXON ENV Vault server usable with any MCP client:
 *   - Claude Desktop
 *   - Cursor
 *   - Windsurf
 ...
 *   - AXON_VAULT_ROOT: Root directory for scanning (default: cwd)
 */
...
async function handleNotification(msg: MCPNotification): Promise<void> {
  if (msg.method === "notifications/initialized") {
    if (!initialized) {
      manager = new EnvVaultManager({
        rootDir: process.env.AXON_VAULT_ROOT || undefined,
      });

      const created = createEnvVaultServer(manager);
      server = created.server;
      store = created.store;
      capAuthority = created.capAuthority;
      initialized = true;
      log("ENV Vault server ready");
    }
  }
...
function log(...args: any[]): void {
  process.stderr.write(`[axon-env-vault] ${args.join(" ")}\n`);
}

async function main(): Promise<void> {
  log("Starting AXON ENV Vault MCP server (stdio)...");
