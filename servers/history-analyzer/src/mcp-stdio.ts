#!/usr/bin/env npx tsx
/**
 * AXON History Analyzer Server — MCP-Compatible Stdio Wrapper
 *
 * Translates MCP JSON-RPC over stdin/stdout into AXON calls.
 * This makes the AXON History Analyzer server usable with any MCP client:
 *   - Claude Desktop
 *   - Cursor
 *   - Windsurf
...
async function handleNotification(msg: MCPNotification): Promise<void> {
  if (msg.method === "notifications/initialized") {
    if (!ha?.isLaunched) {
      ...
      const created = createHistoryAnalyzerServer(ha);
      server = created.server;
      store = created.store;
      capAuthority = created.capAuthority;
    }
    initialized = true;
    log("Chrome launched with user profile, history analyzer server ready");
  }
...
function log(...args: any[]): void {
  process.stderr.write(`[axon-history] ${args.join(" ")}\n`);
}

async function main(): Promise<void> {
  log("Starting AXON History Analyzer MCP server (stdio)...");
