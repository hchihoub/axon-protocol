#!/usr/bin/env npx tsx
/**
 * AXON SSH Key Manager Server — MCP-Compatible Stdio Wrapper
 *
 * Translates MCP JSON-RPC over stdin/stdout into AXON calls.
 * This makes the AXON SSH Key Manager server usable with any MCP client:
 *   - Claude Desktop
 *   - Cursor
 *   - Windsurf
 ...
 *   - AXON_SSH_DIR: Override SSH directory (default: ~/.ssh)
 */
  ...
  if (msg.method === "notifications/initialized") {
    if (!initialized) {
      manager = new SSHKeyManager({
        sshDir: process.env.AXON_SSH_DIR || undefined,
      });

      const created = createSSHKeyServer(manager);
      server = created.server;
      store = created.store;
      capAuthority = created.capAuthority;
      initialized = true;
      log("SSH Key Manager server ready");
    }
  }
...
function log(...args: any[]): void {
  process.stderr.write(`[axon-ssh-keys] ${args.join(" ")}\n`);
}

async function main(): Promise<void> {
  log("Starting AXON SSH Key Manager MCP server (stdio)...");
