#!/usr/bin/env npx tsx
/**
 * AXON System Monitor Server — MCP-Compatible Stdio Wrapper
 *
 * Translates MCP JSON-RPC over stdin/stdout into AXON calls.
 * This makes the AXON System Monitor server usable with any MCP client:
 *   - Claude Desktop
 *   - Cursor
 *   - Windsurf
...
async function handleNotification(msg: MCPNotification): Promise<void> {
  if (msg.method === "notifications/initialized") {
    ...
    log("System monitor server ready");
  }
...
function initServer(): void {
  ...
  const created = createSystemMonitorServer(sm);
...
function log(...args: any[]): void {
  process.stderr.write(`[axon-sysmon] ${args.join(" ")}\n`);
}

async function main(): Promise<void> {
  log("Starting AXON System Monitor MCP server (stdio)...");
