#!/usr/bin/env npx tsx
/**
 * AXON Cookie Manager Server — MCP-Compatible Stdio Wrapper
 *
 * Translates MCP JSON-RPC over stdin/stdout into AXON calls.
 * This makes the AXON Cookie Manager server usable with any MCP client:
 *   - Claude Desktop
 *   - Cursor
 *   - Windsurf
...
async function handleNotification(msg: MCPNotification): Promise<void> {
  if (msg.method === "notifications/initialized") {
    if (!cm?.isLaunched) {
      ...
      const created = createCookieManagerServer(cm);
      server = created.server;
      store = created.store;
      capAuthority = created.capAuthority;
    }
    initialized = true;
    log("Chrome launched with user profile, cookie manager server ready");
  }
...
function log(...args: any[]): void {
  process.stderr.write(`[axon-cookies] ${args.join(" ")}\n`);
}

async function main(): Promise<void> {
  log("Starting AXON Cookie Manager MCP server (stdio)...");
