#!/usr/bin/env npx tsx
/**
 * AXON Bookmarks Manager Server — MCP-Compatible Stdio Wrapper
 *
 * Translates MCP JSON-RPC over stdin/stdout into AXON calls.
 * This makes the AXON Bookmarks Manager server usable with any MCP client:
 *   - Claude Desktop
 *   - Cursor
 *   - Windsurf
...
async function handleNotification(msg: MCPNotification): Promise<void> {
  if (msg.method === "notifications/initialized") {
    if (!bm?.isLaunched) {
      ...
      const created = createBookmarksManagerServer(bm);
      server = created.server;
      store = created.store;
      capAuthority = created.capAuthority;
    }
    initialized = true;
    log("Chrome launched with user profile, bookmarks manager server ready");
  }
...
function log(...args: any[]): void {
  process.stderr.write(`[axon-bookmarks] ${args.join(" ")}\n`);
}

async function main(): Promise<void> {
  log("Starting AXON Bookmarks Manager MCP server (stdio)...");
