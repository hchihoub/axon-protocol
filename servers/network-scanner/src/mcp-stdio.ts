#!/usr/bin/env npx tsx
/**
 * AXON Network Scanner Server — MCP-Compatible Stdio Wrapper
 *
 * Translates MCP JSON-RPC over stdin/stdout into AXON calls.
 * This makes the AXON Network Scanner server usable with any MCP client:
 *   - Claude Desktop
 *   - Cursor
 *   - Windsurf
...
async function handleNotification(msg: MCPNotification): Promise<void> {
  if (msg.method === "notifications/initialized") {
    if (!initialized) {
      ...
      const created = createNetworkScannerServer(scanner);
      server = created.server;
      store = created.store;
      capAuthority = created.capAuthority;
      initialized = true;
      log("Network Scanner server ready");
    }
  }
...
function log(...args: any[]): void {
  process.stderr.write(`[axon-network-scanner] ${args.join(" ")}\n`);
}

async function main(): Promise<void> {
  log("Starting AXON Network Scanner MCP server (stdio)...");
